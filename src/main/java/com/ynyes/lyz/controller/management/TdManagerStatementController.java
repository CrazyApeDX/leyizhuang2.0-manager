    ...
    public String goodsListDialog(String keywords, @PathVariable Long statusId, Integer page, Integer size,
        ...
        if (statusId == 21 || statusId == 22) {
            ...
            while (iterator.hasNext()) {
                ...
                if (!diy.getStoreCode().contains("FX") && !diy.getStoreCode().contains("fx")
                        && !diy.getStoreCode().contains("Fx") && !diy.getStoreCode().contains("fX")) {
                    iterator.remove();
                }
            }
        }
...
//			map.addAttribute("order_page",tdSalesDetailService.queryPageList(begin, end, cityName, diySiteCode, roleDiyIds, size, page));
    ...
    private void setStoreSalesRowValueAndStyle(Object[] cells, HSSFCellStyle cellStyle, HSSFRow row) {
        HSSFCell cell = null;
        if (null != cells && cells.length > 0) {
            for (int i = 0; i < cells.length; i++) {
                cell = row.createCell(i);
                if (i == 9) {
                    switch (Integer.parseInt(cells[i].toString())) {
                        ...
                        case 8:
                            ...
                            break;
                        case 9:
                            cell.setCellValue("退货中");
                            break;
                        case 10:
                            cell.setCellValue("退货确认");
                            break;
                        case 11:
                            cell.setCellValue("退货取消");
                            break;
                        case 12:
                            cell.setCellValue("退货完成");
                            break;
                        default:
                            break;
                    }
...
//        long startTimne = System.currentTimeMillis();
        ...
        for (int i = 0; i < sheets; i++) {
            ...
            for (int j = 0; j < maxRowNum; j++) {
                ...
                row = sheet.createRow((int) j + 1);
                if (null != receipt.getDiySiteName()) {//门店名称
                    row.createCell(0).setCellValue(receipt.getDiySiteName());
                }
                if (null != receipt.getReceiptType()) {//付款类型
                    row.createCell(1).setCellValue(receipt.getReceiptType());
                }
                if (null != receipt.getMainOrderNumber()) {//主单号
                ...
                if (null != receipt.getRealUserRealName()) {//会员名称
...
//        long startTimne = System.currentTimeMillis();
        ...
        for (int i = 0; i < sheets; i++) {
            ...
            for (int j = 0; j < maxRowNum; j++) {
                ...
                row = sheet.createRow((int) j + 1);
                if (returnReport.getDiySiteName() != null) {//退货门店
                    row.createCell(0).setCellValue(returnReport.getDiySiteName());
                }
                if (returnReport.getOrderNumber() != null) {//原订单号
                ...
                if (returnReport.getReturnNumber() != null) {//退货单号
                    row.createCell(2).setCellValue(returnReport.getReturnNumber());
                }
                if (returnReport.getStatusId() != null) {//退货单状态
...
//		            System.out.println("正在生成excel文件的 sheet"+(i+1)+"第"+(j+1)+"行");
    ...
    private HSSFWorkbook FXMemberWorkBook(Date begin, Date end, String diyCode, String cityName, String username, List<String> roleDiyIds) {
        List<Object> FXMemberList = tdUserService.queryFXMemberDownList(begin, end, cityName, diyCode, roleDiyIds);
        HSSFWorkbook wb = new HSSFWorkbook();

        //excel单表最大行数是65535
        int maxRowNum = 60000;
        int maxSize = 0;
        if (FXMemberList != null) {
            maxSize = FXMemberList.size();
        }
        int sheets = maxSize / maxRowNum + 1;
        ...
        for (int i = 0; i < sheets; i++) {
            ...
            // 第三步，在sheet中添加表头第0行,注意老版本poi对Excel的行数列数有限制short
            //列宽
            int[] widths = {15, 15, 15, 15, 20, 15, 20, 20,};
            sheetColumnWidth(sheet, widths);
            // 第四步，创建单元格，并设置值表头 设置表头居中
            HSSFCellStyle style = wb.createCellStyle();
            style.setAlignment(HSSFCellStyle.ALIGN_CENTER); // 创建一个居中格式
            ...
            HSSFRow row = sheet.createRow((int) 0);
            //版本1.1
            String[] cellValues = {"城市", "门店编码", "门店名称", "导购", "导购电话", "会员", "会员电话", "注册时间"};
            cellDates(cellValues, style, row);
            HSSFCellStyle cellStyle = wb.createCellStyle();
            cellStyle.setAlignment(HSSFCellStyle.ALIGN_CENTER);
            for (int j = 0; j < maxRowNum; j++) {
                ...
                Object[] cells = (Object[]) FXMemberList.get(j + i * maxRowNum);
    ...
    private HSSFWorkbook FXStoreSalesWorkBook(Date begin, Date end, String diyCode, String cityName, String username, List<String> roleDiyIds) {
        List<Object> FXStoreSalesList = tdSalesDetailService.queryStoreSalesDownList(begin, end, cityName, diyCode, roleDiyIds);
        HSSFWorkbook wb = new HSSFWorkbook();

        //excel单表最大行数是65535
        int maxRowNum = 60000;
        int maxSize = 0;
        if (FXStoreSalesList != null) {
            maxSize = FXStoreSalesList.size();
        }
        int sheets = maxSize / maxRowNum + 1;
        ...
        for (int i = 0; i < sheets; i++) {
            ...
            // 第三步，在sheet中添加表头第0行,注意老版本poi对Excel的行数列数有限制short
            //列宽
            int[] widths = {15, 15, 15, 15, 20, 15, 20, 20, 15, 20, 15, 20, 15, 20};
            sheetColumnWidth(sheet, widths);
            // 第四步，创建单元格，并设置值表头 设置表头居中
            HSSFCellStyle style = wb.createCellStyle();
            style.setAlignment(HSSFCellStyle.ALIGN_CENTER); // 创建一个居中格式
            ...
            HSSFRow row = sheet.createRow((int) 0);
            //版本1.1
            String[] cellValues = {"城市", "门店编码", "门店名称", "订单号", "下单时间","下单日期", "支付方式", "送货方式", "支付时间", "订单状态", "订单金额", "运费",
            ...
            for (int j = 0; j < maxRowNum; j++) {
                ...
                Object[] cells = (Object[]) FXStoreSalesList.get(j + i * maxRowNum);
