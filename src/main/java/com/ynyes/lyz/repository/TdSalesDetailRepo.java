...
public interface TdSalesDetailRepo extends PagingAndSortingRepository<TdSalesDetail, Long>, JpaSpecificationExecutor<TdSalesDetail> {
    ...
    @Query(value = " SELECT "
            ...
            + " FROM "
            + " 	td_order o "
            + " INNER JOIN td_order_data d on o.main_order_number = d.main_order_number "
            + " WHERE "
            ...
            + " AND o.diy_site_id IN ?5 "
            + " GROUP BY "
            + " IFNULL(o.main_order_number,o.order_number); ", nativeQuery = true)
