...
public interface TdSalesForActiveUserRepo extends
		PagingAndSortingRepository<TdSalesForActiveUser, Long>,
		JpaSpecificationExecutor<TdSalesForActiveUser> {
	...
	/*@Query(value=" SELECT "
			...
			+" 	SELECT "
			+" 		oci.sku coupon_sku, "
			+" 		oci.coupon_type_id coupon_type_id, "
			+" 		oci.price coupon_price, "
			+" 		SUM(oci.quantity) coupon_quantity, "
			+" 		oci.order_header_id "
			+" 	FROM "
			+" 		td_order_coupon_inf oci "
			+" 	GROUP BY "
			+" 		oci.order_header_id, "
			+" 		oci.sku, "
			+"      oci.coupon_type_id "
			+" ) coupon ON ( "
			+" 	ogi.sku = coupon.coupon_sku "
			+" 	AND ogi.order_header_id = coupon.order_header_id "
			+" 	AND ogi.gift_flag = 'N' "
			+" ) "
			+" LEFT JOIN td_diy_site diy ON o.diy_site_name = diy.title "
			+" WHERE "
			+" o.pay_time >= ?1 "
			+" AND o.pay_time <= ?2 "
			...
			+" 	SELECT "
			+" 		o1.order_number order_number "
			+" 	FROM "
			+" 		td_order o1 "
			+" 	INNER JOIN td_order_inf oi1 ON o1.order_number = oi1.order_number "
			+" 	LEFT JOIN td_order_goods_inf ogi1 ON oi1.header_id = ogi1.order_header_id "
			+" 	LEFT JOIN ( "
			+" 		SELECT "
			+" 			oci.sku coupon_sku, "
			+" 			oci.coupon_type_id coupon_type_id, "
			...
			+"			oci.coupon_type_id "
			+" 	) coupon1 ON ( "
			+" 		ogi1.sku = coupon1.coupon_sku "
			+" 		AND ogi1.order_header_id = coupon1.order_header_id "
			+" 		AND ogi1.gift_flag = 'N' "
			+" 	) "
			+" 	LEFT JOIN td_diy_site diy1 ON o1.diy_site_name = diy1.title "
			+" 	WHERE "
			+" 		o1.pay_time >= ?1 "
			+" 	AND o1.pay_time <= ?2 "
			+" 	AND o1.status_id NOT IN (1, 2, 7, 8) "
			+" 	AND ( "
			+" 		( "
			+" 			o1.total_price = 0 "
			+" 			AND o1.all_actual_pay > 0 "
			+" 		) "
			+" 		OR ( "
			+" 			o1.total_price > 0 "
			+" 			AND o1.total_price = o1.other_pay "
			+" 		) "
			+" 	) "
			+" ) "
			+" UNION ALL "
			...
			+" LEFT JOIN td_return_goods_inf rgi ON og.sku = rgi.sku "
			+" AND ri.rt_header_id = rgi.rt_header_id "
			+" LEFT JOIN ( "
			+" 	SELECT "
			+" 		rci.sku coupon_sku, "
			+" 		rci.coupon_type_id, "
			+" 		rci.price coupon_price, "
			+" 		SUM(rci.quantity) coupon_quantity, "
			+" 		rci.rt_header_id "
			+" 	FROM "
			+" 		td_return_coupon_inf rci "
			+" 	GROUP BY "
			+" 		rci.rt_header_id, "
			+" 		rci.sku, "
			+"		rci.coupon_type_id "
			+" ) coupon ON og.sku = coupon.coupon_sku "
	...
	@Query(value=" delete from td_sales_for_active_user where 1=1 ",nativeQuery=true)
	@Transactional
	@Modifying
	void deleteSalesForActiveUser();
