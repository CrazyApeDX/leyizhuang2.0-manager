...
public interface TdGoodsINOutRepo extends
		PagingAndSortingRepository<TdGoodsInOut, Long>,
		JpaSpecificationExecutor<TdGoodsInOut> {
	...
	@Query(value = "select og.id id,o.diy_site_name diy_site_name,o.main_order_number main_order_number,o.order_number order_number,o.order_time order_time, "
			...
			+ "  where o.deliver_type_title = '送货上门' and di.end_dt>=?1 and di.end_dt<=?2 and o.status_id not in(1,2,7,8) and o.city like ?3 and o.diy_site_code like ?4 and o.diy_site_id in ?5 "
			+ "union ALL "
			...
			+ "wh.wh_name wh_name,u1.real_name deliver_real_name,u1.username deliver_username,o.shipping_name shipping_name,o.shipping_phone shipping_phone,"
			...
			+ "left join td_product_category ppc on ppc.id=pc.parent_id "
			+ " where o.deliver_type_title = '送货上门' and ((rn.receive_time >= ?1 AND rn.receive_time <= ?2 ) OR (rn.recv_time >= ?1 AND  rn.recv_time <= ?2 )) and o.status_id not in(1,2,7,8) and o.city like ?3 and o.diy_site_code like ?4 and o.diy_site_id in ?5 "
			+ "order by sales_time desc;", nativeQuery = true)
	List<TdGoodsInOut> queryDownList(Date begin, Date end, String cityName,
