...
public interface TdReconciliationRepo extends PagingAndSortingRepository<TdReconciliation, Long>,
	...
	@Query(value = "    SELECT * FROM (SELECT      "
			...
			+"    -1*od.pro_coupon_fee pro_coupon_fee,    "
			+"    -1*od.cash_coupon_fee cash_coupon_fee,    "
			...
			+"    SELECT      "
			+" 	  UUID() id,    "
			+"    (CASE WHEN r.remark_info = '拒签退货' OR r.remark_info = '拒签退货' THEN r.order_time ELSE cr.finish_time END) order_time,    "
			+"    u.id user_id,    "
			...
			+"    -1*l.money balance_used,    "
			+"    -1*(CASE WHEN cr.type_title = '支付宝' THEN cr.money ELSE 0 END) ali_pay,    "
			+"    -1*(CASE WHEN cr.type_title = '微信' OR cr.type_title = '微信支付' THEN cr.money ELSE 0 END) wechat_pay,    "
			+"    -1*(CASE WHEN cr.type_title = '银行卡' THEN cr.money ELSE 0 END) union_pay,    "
			+"    -1*(CASE WHEN locate('YF', r.order_number) THEN r.turn_price ELSE 0 END) delivery_fee,    "
			+"    0 left_price,    "
			+"    0 delivery_cash,    "
			+"    0 delivery_pos,    "
			+"    -1*(CASE WHEN cr.type_title = '门店现金' THEN cr.money ELSE 0 END) seller_cash,    "
			+"    0 seller_pos,    "
			+"    0 seller_other,    "
			+"    0 due,    "
			+"    wh.wh_name wh_name,    "
