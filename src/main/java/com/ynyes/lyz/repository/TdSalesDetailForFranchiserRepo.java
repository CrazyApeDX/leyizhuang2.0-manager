package com.ynyes.lyz.repository;

import java.util.Date;
import java.util.List;

import org.springframework.data.jpa.repository.JpaSpecificationExecutor;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.PagingAndSortingRepository;

import com.ynyes.lyz.entity.TdSalesDetailForFranchiser;

/**
 * 预存款变更商品明细表 dao层
 * 
 * @author Richard
 *
 */

public interface TdSalesDetailForFranchiserRepo extends PagingAndSortingRepository<TdSalesDetailForFranchiser, Long>, JpaSpecificationExecutor<TdSalesDetailForFranchiser> {
	
	
	/**
	 * 查询下载的list
	 * @param begin 开始时间
	 * @param end 结束时间
	 * @param cityName 城市
	 * @param diySiteCode 门店
	 * @param roleDiyIds 权限门店
	 *
	 */
	@Query(value =" SELECT DISTINCT "
			+" 	UUID() id, "
			+" 	o.diy_site_name diy_site_name, "
			+" 	o.main_order_number main_order_number, "
			+" 	o.order_number order_number, "
			+" 	o.pay_time order_time, "
			+" 	o.status_id status_id, "
			+" 	su.real_name seller_real_name, "
			+" 	o.user_id, "
			+" 	u.real_name real_name, "
			+" 	IFNULL( "
			+" 		o.real_user_username, "
			+" 		o.username "
			+" 	) username, "
			+" 	og.sku sku, "
			+" 	og.goods_title goods_title, "
			+" 	og.quantity quantity, "
			+" 	CASE "
			+" WHEN IFNULL(o.dif_fee, 0) > 0 THEN "
			+" 	IFNULL(og.real_price, 0) "
			+" ELSE "
			+" 	og.price "
			+" END AS price, "
			+"  og.quantity * og.price total_price, "
			+"  IFNULL(og.jx_price, 0) jx_price, "
			+"  IFNULL( coupon.quantity,0) product_coupon_quantity, "
			+"  og.brand_title brand_title, "
			+"  ppc.title goods_parent_type_title, "
			+"  pc.title goods_type_title, "
			+"  o.deliver_type_title deliver_type_title, "
			+"  wh.wh_name wh_name, "
			+"  u1.real_name deliver_real_name, "
			+"  u1.username deliver_username, "
			+"  o.shipping_name shipping_name, "
			+"  o.shipping_phone shipping_phone, "
			+"  o.shipping_address shipping_address, "
			+"  o.remark remark, "
			+"  diy.city city_name, "
			+"  o.diy_site_code diy_site_code, "
			+"  o.diy_site_id diy_id "
			+" FROM "
			+" 	td_order o "
			+" INNER JOIN td_order_goods og ON ( "
			+" 	og.td_order_id = o.id "
			+" 	OR og.presented_list_id = o.id "
			+" 	OR og.gift_list_id = o.id "
			+" ) "
			+" LEFT JOIN td_user u ON u.username = IFNULL( "
			+" 	o.real_user_username, "
			+" 	o.username "
			+" ) "
			+" LEFT JOIN td_product_category pc ON pc.id = ( "
			+" 	SELECT "
			+" 		a.category_id "
			+" 	FROM "
			+" 		td_goods a "
			+" 	WHERE "
			+" 		a.id = og.goods_id "
			+" ) "
			+" LEFT JOIN td_product_category ppc ON ppc.id = pc.parent_id "
			+" LEFT JOIN td_delivery_info di ON di.order_number = o.main_order_number "
			+" LEFT JOIN td_user u1 ON u1.op_user = di.driver "
			+" LEFT JOIN td_ware_house wh ON wh.wh_number = di.wh_no "
			+" LEFT JOIN td_user su ON su.id = o.seller_id "
			+" LEFT JOIN td_diy_site diy ON o.diy_site_id = diy.id "
			+" LEFT JOIN(SELECT "
			+" 	oi.order_number order_number, "
			+" 	oi.header_id order_header_id, "
			+" 	ogi.sku sku, "
			+" 	sum(oci.quantity) quantity "
			+" FROM "
			+" 	td_order o "
			+" LEFT JOIN td_order_goods og ON o.id = og.td_order_id "
			+" LEFT JOIN td_order_inf oi ON o.order_number = oi.order_number "
			+" LEFT JOIN td_order_goods_inf ogi ON ( "
			+" 	oi.header_id = ogi.order_header_id "
			+" 	AND og.td_order_id IS NOT NULL "
			+" 	AND og.sku = ogi.sku "
			+" 	AND ogi.gift_flag = 'N' "
			+" ) "
			+" LEFT JOIN td_order_coupon_inf oci ON ogi.order_header_id = oci.order_header_id "
			+" AND ogi.sku = oci.sku "
			+" WHERE "
			+" 	oi.init_date >=  date_sub(?1,interval 1 MONTH) "
			+" AND oci.coupon_type_id IN (1, 4) "
			+" GROUP BY "
			+" 	oci.order_header_id, "
			+" 	oci.sku) coupon on o.order_number = coupon.order_number AND og.sku = coupon.sku "
			+" WHERE "
			+" 	o.status_id > 2 "
			+" AND o.is_coupon IS NULL "
			+" AND o.pay_time >= ?1 "
			+" AND o.pay_time <= ?2 "
			+" AND diy.city LIKE ?3 "
			+" AND o.diy_site_code LIKE ?4   "
			+" AND o.diy_site_id IN ?5 "
			+"  "
			+" UNION ALL "
			+"  "
			+" SELECT "
			+" 	UUID() id, "
			+" 	o.diy_site_name diy_site_name, "
			+" 	rn.return_number main_order_number, "
			+" 	rn.order_number order_number, "
			+" 	rn.order_time order_time, "
			+" 	rn.status_id status_id, "
			+" 	o.seller_real_name seller_real_name, "
			+" 	o.user_id user_id, "
			+" 	u.real_name real_name, "
			+" 	rn.username username, "
			+" 	og.sku sku, "
			+" 	og.goods_title goods_title, "
			+" 	- og.quantity quantity, "
			+" 	CASE "
			+" WHEN IFNULL(o.dif_fee, 0) > 0 THEN "
			+" 	IFNULL(og.real_price, 0) "
			+" ELSE "
			+" 	og.price "
			+" END AS price, "
			+"  0 total_price, "
			+"  IFNULL(og.jx_price, 0) jx_price, "
			+"  IFNULL( "
			+" 	- coupon.quantity, "
			+" 	0 "
			+" ) product_coupon_quantity, "
			+"  og.brand_title brand_title, "
			+"  ppc.title goods_parent_type_title, "
			+"  pc.title goods_type_title, "
			+"  o.deliver_type_title deliver_type_title, "
			+"  wh.wh_name wh_name, "
			+"  u1.real_name deliver_real_name, "
			+"  u1.username deliver_username, "
			+"  o.shipping_name shipping_name, "
			+"  o.shipping_phone shipping_phone, "
			+"  o.shipping_address shipping_address, "
			+"  rn.remark_info remark, "
			+"  o.city city_name, "
			+"  o.diy_site_code diy_site_code, "
			+"  o.diy_site_id diy_id "
			+" FROM "
			+" 	td_return_note rn "
			+" INNER JOIN td_order o ON o.order_number = rn.order_number "
			+" INNER JOIN td_order_goods og ON ( "
			+" 	og.td_order_id = o.id "
			+" 	OR og.presented_list_id = o.id "
			+" 	OR og.gift_list_id = o.id "
			+" ) "
			+" LEFT JOIN td_user u ON u.username = o.username "
			+" LEFT JOIN td_delivery_info di ON di.order_number = o.main_order_number "
			+" LEFT JOIN td_user u1 ON u1.op_user = rn.driver "
			+" LEFT JOIN td_ware_house wh ON wh.wh_number = di.wh_no "
			+" LEFT JOIN td_goods g ON g.id = og.goods_id "
			+" LEFT JOIN td_product_category pc ON pc.id = g.category_id "
			+" LEFT JOIN td_product_category ppc ON ppc.id = pc.parent_id "
			+" LEFT JOIN td_diy_site diy ON o.diy_site_id = diy.id "
			+" LEFT JOIN ( "
			+" 	SELECT "
			+" 		ri.return_number order_number, "
			+" 		ri.rt_header_id order_header_id, "
			+" 		rgi.sku sku, "
			+" 		sum(rci.quantity) quantity "
			+" 	FROM "
			+" 		td_return_note rn "
			+" 	LEFT JOIN td_order_goods og ON rn.id = og.td_return_id "
			+" 	LEFT JOIN td_return_order_inf ri ON rn.return_number = ri.return_number "
			+" 	LEFT JOIN td_return_goods_inf rgi ON ( "
			+" 		ri.rt_header_id = rgi.rt_header_id "
			+" 		AND og.td_return_id IS NOT NULL "
			+" 		AND og.sku = rgi.sku "
			+" 	) "
			+" 	LEFT JOIN td_return_coupon_inf rci ON rgi.rt_header_id = rci.rt_header_id "
			+" 	AND rgi.sku = rci.sku "
			+" 	WHERE "
			+" 		ri.init_date >= date_sub(?1, INTERVAL 1 WEEK) "
			+" 	AND rci.coupon_type_id IN (1, 4) "
			+" 	GROUP BY "
			+" 		rci.rt_header_id, "
			+" 		rci.sku "
			+" ) coupon ON rn.return_number = coupon.order_number "
			+" AND og.sku = coupon.sku "
			+" WHERE "
			+" 	rn.remark_info IN ( "
			+" 		'用户取消订单，退货', "
			+" 		'拒签退货' "
			+" 	) "
			+" AND o.is_coupon IS NULL "
			+" AND rn.order_time >= ?1 "
			+" AND rn.order_time <= ?2 "
			+" AND diy.city LIKE ?3 "
			+" AND o.diy_site_code LIKE ?4  "
			+" AND o.diy_site_id IN ?5 "
			+" 	 "
			+" UNION ALL "
			+"  "
			+" SELECT DISTINCT "
			+" 	UUID() id, "
			+" 	o.diy_site_name diy_site_name, "
			+" 	rn.return_number main_order_number, "
			+" 	rn.order_number order_number, "
			+" 	rn.return_time order_time, "
			+" 	rn.status_id status_id, "
			+" 	o.seller_real_name seller_real_name, "
			+" 	o.user_id user_id, "
			+" 	u.real_name real_name, "
			+" 	rn.username username, "
			+" 	og.sku sku, "
			+" 	og.goods_title goods_title ,- og.quantity quantity, "
			+" 	og.price ,- og.quantity * og.price total_price, "
			+" 	IFNULL(og.jx_price, 0) jx_price, "
			+" 	IFNULL( "
			+" 		- coupon.quantity, "
			+" 		0 "
			+" 	) product_coupon_quantity, "
			+" 	og.brand_title brand_title, "
			+" 	ppc.title goods_parent_type_title, "
			+" 	pc.title goods_type_title, "
			+" 	o.deliver_type_title deliver_type_title, "
			+" 	wh.wh_name wh_name, "
			+" 	u1.real_name deliver_real_name, "
			+" 	u1.username deliver_username, "
			+" 	o.shipping_name shipping_name, "
			+" 	o.shipping_phone shipping_phone, "
			+" 	o.shipping_address shipping_address, "
			+" 	rn.remark_info remark, "
			+" 	o.city city_name, "
			+" 	o.diy_site_code diy_site_code, "
			+" 	o.diy_site_id diy_id "
			+" FROM "
			+" 	td_return_note rn "
			+" INNER JOIN td_order_goods og ON og.td_return_id = rn.id "
			+" INNER JOIN td_order o ON o.order_number = rn.order_number "
			+" LEFT JOIN td_back_main back ON rn.return_number = back.po_no "
			+" LEFT JOIN td_user u ON u.username = o.username "
			+" LEFT JOIN td_user u1 ON u1.op_user = rn.driver "
			+" LEFT JOIN td_ware_house wh ON wh.wh_number = back.wh_no "
			+" LEFT JOIN td_goods g ON g.id = og.goods_id "
			+" LEFT JOIN td_product_category pc ON pc.id = g.category_id "
			+" LEFT JOIN td_product_category ppc ON ppc.id = pc.parent_id "
			+" LEFT JOIN td_diy_site diy ON o.diy_site_id = diy.id "
			+" LEFT JOIN ( "
			+" 	SELECT "
			+" 		ri.return_number order_number, "
			+" 		ri.rt_header_id order_header_id, "
			+" 		rgi.sku sku, "
			+" 		sum(rci.quantity) quantity "
			+" 	FROM "
			+" 		td_return_note rn "
			+" 	LEFT JOIN td_order_goods og ON rn.id = og.td_return_id "
			+" 	LEFT JOIN td_return_order_inf ri ON rn.return_number = ri.return_number "
			+" 	LEFT JOIN td_return_goods_inf rgi ON ( "
			+" 		ri.rt_header_id = rgi.rt_header_id "
			+" 		AND og.td_return_id IS NOT NULL "
			+" 		AND og.sku = rgi.sku "
			+" 	) "
			+" 	LEFT JOIN td_return_coupon_inf rci ON rgi.rt_header_id = rci.rt_header_id "
			+" 	AND rgi.sku = rci.sku "
			+" 	WHERE "
			+" 		ri.init_date >= date_sub(?1, INTERVAL 1 WEEK) "
			+" 	AND rci.coupon_type_id IN (1, 4) "
			+" 	GROUP BY "
			+" 		rci.rt_header_id, "
			+" 		rci.sku "
			+" ) coupon ON rn.return_number = coupon.order_number "
			+" AND og.sku = coupon.sku "
			+" WHERE "
			+" 	( "
			+" 		rn.remark_info NOT IN ( "
			+" 			'用户取消订单，退货', "
			+" 			'拒签退货' "
			+" 		) "
			+" 		OR rn.remark_info IS NULL "
			+" 	) "
			+" AND o.is_coupon IS NULL "
			+" AND rn.return_time >= ?1 "
			+" AND rn.return_time <= ?2 "
			+" AND diy.city LIKE ?3 "
			+" AND o.diy_site_code LIKE ?4  "
			+" AND o.diy_site_id IN ?5 "
			+" ORDER BY "
			+" 	order_time DESC; ",nativeQuery = true)
	List<TdSalesDetailForFranchiser> queryDownList(Date begin,Date end,String cityName,String diySiteCode,List<String> roleDiyIds);
}
