...
public interface TdSalesRepo extends
    ...
    @Query(value = " SELECT temp.* FROM (SELECT "
            ...
            + "   WHEN 'N' "
            + "     THEN "
            + "      CASE WHEN o.pay_time>= '2017-10-08 21:00:00 '"
            + "      THEN ogi.hy_price   "
            + "      ELSE "
            + "        ogi.ls_price "
            + "      END "
            ...
            + "   LEFT JOIN td_user u ON u.username = o.real_user_username "
            + " WHERE "
            + "   diy.city LIKE ?3 "
            + "   AND o.pay_time >= ?1 "
            + "   AND o.pay_time <= ?2 "
            ...
            + " WHERE "
            + "   diy.city LIKE ?3 "
            + "   AND (CASE WHEN rn.remark_info ='用户取消订单，退货' "
            + "     THEN rn.order_time >= ?1 "
            + "        ELSE rn.receive_time >= ?1 "
            + "        END) "
            + "   AND (CASE WHEN rn.remark_info ='用户取消订单，退货' "
            + "     THEN rn.order_time <= ?2 "
            + "        ELSE rn.receive_time <= ?2 "
            + "        END) "
            + "  AND (CASE WHEN rn.remark_info ='用户取消订单，退货'  "
            + "     THEN o.total_price - o.other_pay = 0 "
            + "       ELSE 1=1   "
            + "        END ) "
            + "   AND o.diy_site_code LIKE ?4 "
            + "   AND o.diy_site_id IN ?5 "
