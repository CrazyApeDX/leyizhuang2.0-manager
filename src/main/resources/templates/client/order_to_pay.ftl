...
<html lang="zh-CN" class="bgc-f3f4f6">
    <head>
        ...
        <script type="text/javascript" src="/client/js/order_to_pay.js"></script>
    </head>
        ...
        <article class="balance-recharge">
            ...
            <#if pay_type_list??>
                <section class="pay-way">
                    ...
                    <a class="btn-next" href="javascript:payNow();">确定</a>
                </section>
            </#if>
            <#-- 使用一个隐藏标签用于存储当前选中的支付方式 -->
            <input type="hidden" id="selectedType" value="<#if payTypeId??>${payTypeId?c}<#else>0</#if>">
            <#-- 使用一个隐藏标签用于存储当前支付的订单 -->
            <input type="hidden" id="order" value="<#if orderId??>${orderId?c}<#else>0</#if>">
        </article>
