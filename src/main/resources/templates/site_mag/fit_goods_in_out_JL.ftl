...
function downloaddate(type)
{
    ...
    var salasId = $("#salasId").val();
    if(begain == "")
    {
        alert("请选择开始时间！");
        return;
    }
    window.open("/Verwalter/statement/downdata?begindata="+ begain + "&enddata=" + end+"&code="+code+"&cityName="+city+"&statusId="+type+"&salasId="+salasId);
}
    ...
    <div class="toolbar-wrap">
        <div id="floatHead" class="toolbar">
				...
				<div class="r-list" style="width: 100%;">
					...
					<div class="odiv" style="float: left; width: 310px;">
						...
						<div class="rule-single-select">
							<select name="cityName" id="cityName" onchange="javascript:setTimeout(__doPostBack('changeCity',''), 0)">
								...
								<option sobId ="${city.sobIdCity}" value="${city.cityName }"<#if cityName?? && cityName==city.cityName>selected</#if> >${city.cityName }</option>
								</#list>
							</select>
						</div>
					</div>
					...
					<div class="odiv" style="float: left; width: 310px;">
						<span class="span1">销售经理名称：</span>
						<div class="rule-single-select">
							<select name="salasId" id="salasId" onchange="javascript:setTimeout(__doPostBack('changeSalas',''), 0)">
								<option value="-1">请选择</option> 
								<#list fitSalesManagers as item>
								<option value="${item.id }"<#if salasId?? && salasId==item.id>selected</#if> >${item.name }</option>
								</#list>
							</select>
						</div>
					</div>
					...
					<#if fit_company_ist?? && fit_company_ist?size gt 0>
					<div class="odiv" style="float: left; width: 310px;">
						<span class="span1">装饰公司名称：</span>
						<div class="rule-single-select">
							<select name="code" id="code" onchange="javascript:setTimeout(__doPostBack('changeFit',''), 0)">
								<option value="" >请选择</option> 
								<#list fit_company_ist as fitCompnay>
								<option value="${fitCompnay.code }"<#if code?? && code==fitCompnay.code>selected</#if> >${fitCompnay.name }</option>
								</#list>
							</select>
						</div>
					</div>
