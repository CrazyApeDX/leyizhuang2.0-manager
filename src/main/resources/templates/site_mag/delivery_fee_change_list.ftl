...
<body class="mainbody">
<form name="form1" method="post" action="/Verwalter/delivery/fee/change/list/" id="form1">
<div>
...
<tbody>
    ...
    <#if order_page??>
        <#list order_page.content as order>
        	<#if !(order.isCoupon??&&order.isCoupon) && "送货上门"==order.deliverTypeTitle>
	            <tr>
	                <td align="center">
	                ...
	                <td>
	                    <a href="/Verwalter/delivery/fee/change/edit?id=${order.id?c}">${order.orderNumber!""}<#if order.isCoupon??&&order.isCoupon>（券订单）</#if></a></td>
