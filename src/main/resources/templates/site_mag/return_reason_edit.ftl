...
<body class="mainbody">
<form method="post" action="/Verwalter/returnReason/save" id="form1">
<div>
...
-->
<input name="id" type="text" value='<#if reason??>${reason.id?c!""}</#if>' style="display:none">
    <!--导航栏-->
    <div class="location">
        ...
        <i class="arrow"></i>
        <a href="/Verwalter/returnReturn/list"><span>
            退货原因</span></a> <i class="arrow"></i><span>编辑退货原因</span>
    </div>
    <div class="line10">
    </div>
    <!--/导航栏-->
    ...
    <div class="tab-content" style="display: block;">
        ...
        <dl>
            <dt>退货原因</dt>
            <dd>
                <input id="idBrandTitle" name="title" type="text" datatype="*" value="<#if reason??>${reason.title!''}</#if>" <#--ajaxurl="/Verwalter/returnReason/check<#if reason??>?id=${reason.id?c}</#if>"--> class="input normal" sucmsg=" " />
		...
		<dl>
            <dt>备注</dt>
            <dd>
                <input name="remark" type="text" value="<#if reason??>${reason.remark!""}</#if>" class="input normal">
        ...
        <dl>
            <dt>排序数字</dt>
            <dd>
                <input name="sortId" type="text" value="<#if brand??>${reason.sortId!""}<#else>99</#if>" class="input txt100" datatype="/^(([1-9]\d{0,1})|0)((\.\d{2})|(\.\d{1}))?$/" sucmsg=" ">
