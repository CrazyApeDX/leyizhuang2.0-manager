...
<form name="form1" method="post" action="/Verwalter/order/save" id="form1">
	                            	...
	                            	var changeDeliveryFee = function() {
	                            		$.dialog.prompt('修改运费为：', function(fee) {
	                            			if (isNaN(fee)) {
												...
												$.ajax({
													url: '/Verwalter/delivery/fee/change/save',
													method: 'POST',
													data: {
														fee: fee,
														id: ${order.id?c}
													...
													success: function(result) {
														if (0 === result.status) {
															window.location.href = "/Verwalter/delivery/fee/change/edit?id=${order.id?c}&statusId=0";
														}
													}
												});	                            			
	                            			}
    ...
    <div class="page-footer">
        <div class="btn-list">
            <input type="button" value="返回上一页" class="btn yellow" onclick="window.location.href='/Verwalter/delivery/fee/change/list'">
        </div> 
        <div class="clear">
        </div>
    </div>
