...
<form name="form1" method="post" action="/Verwalter/order/save" id="form1">
        ...
        <dl>
            ...
            <#else>
            	<tr>
                    ...
                    <tr>
                    	<th>活动立减</th>
                        <td><#if order.activitySubPrice??>${order.activitySubPrice?string("0.00")}<#else>0.00</#if>元</td>
                    </tr>
                    <tr>
                    	<th>收货人是否是主家</th>
	...
	<div class="dialog">
		...
		<div class="dialog_row"><input placeholder="POS刷卡交易单号后六位" id="serialNumber" style="height:30px;width:175px;" type="number" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/> </div>
		...
		<div class="dialog_row"><input onclick="sumbitBackMoney(<#if ownPrice??>${ownPrice?c}<#else>0</#if>,${order.id?c })" class="dialog_btn btn" id="ascertain" type="button" value="确定" /><input onclick="hiddenDialog()" class="dialog_btn btn" type="button" value="取消" /> </div>
	</div>
